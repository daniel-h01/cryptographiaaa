...
Polynomial operator* (const Polynomial& lhs_, const Polynomial& rhs) {
    Polynomial lhs(lhs_);
    return lhs *= rhs;
}
...
public:
    Number a;
    Number p;
    Z_p(const Number a, const Number p) : a(a), p(p){};

    Z_p operator*(Z_p rhs)
    {
...
intint main(){
    ...
    for (char c : str)
    {
        num.push_back(char_to_number(c));
    }
